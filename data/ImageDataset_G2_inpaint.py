...
class ImageDataset_G2_i(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(self.image_fnames):
            ...
            if self.phase == Phase.TRAIN:
                puncture_percent = random.uniform(0.0, 0.5)
            else:
                puncture_percent = 0.6

            # final_mask HAS percentage(eg. 20%) 0s AND THE REST 1s
            # FINAL_MASK HAS 0s IN PLACES FOR WHICH HR INFO IS MISSING
            final_mask, total_masks = Mask_Utils.create_mask_for_puncture(self.current_scale, puncture_percent, self.tile_res)
            ...
            metadata = ret_data['mask']*ret_data['tip']

            ret_data['composite'] = ret_data['hr']*final_mask + ret_data['bicubic']*(1 - final_mask)

            # WITH METADATA
            ret_data['g1_input'] = torch.cat((ret_data['composite'],metadata),0)

            #WITHOUT METADATA
            #ret_data['g1_input'] = ret_data['composite']
            #print("SIZES", metadata.size(), ret_data['bicubic'].size())
