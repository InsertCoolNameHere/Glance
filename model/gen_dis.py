...
from model.layers import PixelShuffleUpsampler,Conv2d,CompressionBlock,init_weights,DenseConnectedBlock,DiscriminatorBlock, ResnetBlock
...
class HierarchicalGenerator(pl.LightningModule):
    ...
    def forward(self, x, label, upscale_factor=None, blend=1.0):
        ...
        print(type(label))
        print(label)
        print(type(self.label_embedding))
        print(self.label_embedding)'''

        #print("RIKI ISCUDA?", label.is_cuda)
        lab_emb = self.label_embedding(label)
        #print("LAB", lab_emb.size())
        lab_emb = self.lin_act(lab_emb)
        #print("LABEL", lab_emb.size())
        lab_emb = torch.reshape(lab_emb, (num_labels, 1, self.lr_res, self.lr_res))
